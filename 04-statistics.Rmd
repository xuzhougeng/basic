...
```{r boxplot, fig.cap="箱线图", fig.align='center'}
# 使用箱线图展示某一列数据的分布情况
boxplot(iris$Sepal.Length)
```


```{r histplot, fig.cap="直方图", fig.align='center'}
# 使用直方图展示某一列数据的分布情况
hist(iris$Sepal.Length)
```


```{r ecdf, fig.cap="累积分布函数图", fig.align='center'}
...
```{r normDist, fig.cap="正态分布检验", fig.align='center', echo=FALSE}
knitr::include_graphics("image/C4/NORM_DIST_TEST.png")
```
...
```
```{r xiangguang, fig.cap="相关性展示", fig.align='center', echo=FALSE}
knitr::include_graphics("image/C4/scatterPlot.png")
```
...
```{r circle, fig.cap="GRID绘图系统展示", fig.align='center'}
# 图形来自于R会图形系统（第二版）
library(grid)
grid.newpage()
...
```{r multi-plot, fig.cap="一页多图", fig.align='center'}
library(ggplot2)
grid.newpage()
...
```{r exprMatrix, message=FALSE, warning=FALSE,echo=FALSE}
data("sCLLex")
sCLLex <- sCLLex[,1:8] # 样本数过多，仅选取前8个
group_List <- sCLLex$Disease # 获取分组信息
exprSet <- exprs(sCLLex) # 获取表达矩阵
knitr::kable(head(exprSet, n=3),caption = "表达矩阵的前三行")
```
...
完成了上述准备工作后，先用箱线图，小提琴图，柱状图，密度图了解各个样本表达数据的分布情况。
```{r distribution, fig.cap="箱线图和小提琴图",fig.align="center"}
library(ggplot2)
library(grid)
grid.newpage()
...
其中小提琴图可以认为是箱线图与核密度图的结合体。

```{r densityPlot,fig.cap="条形图和密度图",fig.align="center"}
library(ggplot2)
# 条形图
...
```{r allInOne,fig.cap="不同分组数据在同一幅图中的分布情况",fig.align="center"}
...
```{r gpairs, warning=FALSE,fig.cap="配对关系图",fig.align="center"}
library(gpairs)
gpairs(exprSet[,1:8]
...
```{r cluster,fig.cap="欧几里得距离聚类图",fig.align="center"}
...
```{r heatmap1,fig.cap="热图展示样本间差异",fig.align="center"}
...
```{r heatmap2,fig.cap="热图表征基因表达模式",fig.align="center"}
# 选择表达量较高的基因
...
```{r heatmap3,fig.cap="合理配色的重要性",fig.align="center"}
heatmap(choose_matrix)
```

**PCA**: PCA本质是散点图，X和Y轴表示不同主成分，不同点之间的距离表示样本之间在两个主成分下的差异。
```{r PCA,fig.cap="PCA",fig.align="center"}
...
```{r valnco,fig.cap="火山图",fig.align="center"}
...
```{r valnco2,fig.cap="火山图部分高亮",fig.align="center"}
...
```{r structure,fig.cap="根据GTF画基因的多个转录本结构",fig.align="center"}
